...
/**
 * Perform a Tweets by User search using the Desearch client.
 *
 ...
 */
async function performTweetsByUserSearch() {
    try {
        // Perform the Tweets by User search
        const tweetsByUser = await desearch.tweetsByUser({
            user: "elonmusk",
            query: "Bittensor",
            count: 10
        });
        ...
        console.error('Error performing Tweets by User search:', error);
    }
}

// Call the function to perform the basic Twitter search
performTweetsByUserSearch();